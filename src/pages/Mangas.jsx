...
function Mangas() {
  ...
  const carregarMangas = async () => {
    ...
    try {
      ...
      console.error(err); 
      setErro("Erro ao carregar mangas. Por favor, tenta novamente mais tarde (Rate Limit).");
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="max-w-container mx-auto px-4 px-md-5 pt-5 mt-4">
          ...
          <form onSubmit={handlePesquisa} className="d-flex flex-column flex-md-row gap-3">
            ...
            <div className="d-flex gap-3">
              ...
              >
                ...
                <option value="chapters" style={{ color: '#000' }}>Most Chapters</option>
              </select>
        ...
        <div className="row row-cols-2 row-cols-sm-3 row-cols-md-4 row-cols-lg-5 g-4">
          {loading ? (
            ...
            !erro && mangas.map((manga) => (
              <div key={manga.mal_id} className="col">
                <Link to={`/detalhes/manga/${manga.mal_id}`} className="text-decoration-none">
                  <div className="rounded-4 overflow-hidden bg-surface h-100 transition-all hover-glow border" style={{ borderColor: 'var(--border-color)', boxShadow: 'var(--shadow-card)' }}>
                    <div className="position-relative" style={{ aspectRatio: '2/3' }}>
                      <img 
                        src={manga.images?.jpg?.large_image_url || manga.images?.jpg?.image_url || PLACEHOLDER_IMG} 
                        alt={manga.title} 
                      ...
                      <div className="position-absolute top-0 end-0 p-2">
                        <span className="badge rounded-pill" style={{ backgroundColor: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)' }}>
                          <i className="bi bi-star-fill text-warning" style={{ fontSize: '0.7rem' }}></i> {manga.score ? manga.score.toFixed(1) : 'N/A'}
                        </span>
                      </div>
                    </div>
                    <div className="p-3">
                      <h3 className="text-white mb-1 fw-bold text-truncate" style={{ fontSize: '0.95rem' }}>{manga.title}</h3>
                      <p className="text-muted m-0 small">{manga.published?.prop?.from?.year || manga.type}</p>
                    </div>
                  </div>
                </Link>
              </div>
